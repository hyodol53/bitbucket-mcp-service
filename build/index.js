    ...
    try {
        switch (name) {
            case "get_repository_info": {
                const { repo_slug, workspace } = args;
            ...
            case "get_commits": {
                const { repo_slug, workspace, branch, limit = 10 } = args;
                const actualWorkspace = workspace || BITBUCKET_DEFAULT_WORKSPACE;
                if (!actualWorkspace) {
                    throw new McpError(ErrorCode.InvalidParams, "Workspace is required. Either provide workspace parameter or set BITBUCKET_DEFAULT_WORKSPACE environment variable.");
                }
            ...
            case "get_commit_detail": {
                const { repo_slug, commit_id, workspace } = args;
                const actualWorkspace = workspace || BITBUCKET_DEFAULT_WORKSPACE;
                if (!actualWorkspace) {
                    throw new McpError(ErrorCode.InvalidParams, "Workspace is required. Either provide workspace parameter or set BITBUCKET_DEFAULT_WORKSPACE environment variable.");
                }
            ...
            case "get_pull_requests": {
                const { repo_slug, workspace, state = "OPEN" } = args;
                const actualWorkspace = workspace || BITBUCKET_DEFAULT_WORKSPACE;
                if (!actualWorkspace) {
                    throw new McpError(ErrorCode.InvalidParams, "Workspace is required. Either provide workspace parameter or set BITBUCKET_DEFAULT_WORKSPACE environment variable.");
                }
            ...
            case "get_file_content": {
                const { repo_slug, file_path, workspace, branch = "main" } = args;
                const actualWorkspace = workspace || BITBUCKET_DEFAULT_WORKSPACE;
                if (!actualWorkspace) {
                    throw new McpError(ErrorCode.InvalidParams, "Workspace is required. Either provide workspace parameter or set BITBUCKET_DEFAULT_WORKSPACE environment variable.");
                }
            ...
            case "get_directory_content": {
                const { repo_slug, workspace, directory_path = "", branch = "main" } = args;
                const actualWorkspace = workspace || BITBUCKET_DEFAULT_WORKSPACE;
                if (!actualWorkspace) {
                    throw new McpError(ErrorCode.InvalidParams, "Workspace is required. Either provide workspace parameter or set BITBUCKET_DEFAULT_WORKSPACE environment variable.");
                }
